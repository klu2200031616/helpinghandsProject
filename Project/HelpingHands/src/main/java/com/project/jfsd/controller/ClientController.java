...
import com.project.jfsd.service.EventService;
import com.project.jfsd.service.UserService;
...
public class ClientController {
	...
	@GetMapping("/aboutus")
	public ModelAndView aboutusRedirect() {
	...
	@GetMapping("/donate")
	public ModelAndView donateRedirect() {
		return new ModelAndView("donate");
	}
	
	@GetMapping("/volunteer")
	public ModelAndView volunteerRedirect() {
		return new ModelAndView("volunteer");
	}
	
	@GetMapping("/volunteerregister")
	public ModelAndView volunteerregisterRedirect() {
		return new ModelAndView("volunteerregister");
	}
	
	@GetMapping("/pickup")
	public ModelAndView pickup() {
		return new ModelAndView("pickup");
	}
	...
	public ModelAndView login(@ModelAttribute("user") User loginUser, HttpServletRequest request) {
	    ...
	    if (isAuthenticated) {
	        ...
	        if (user != null) {
	            ...
	            modelAndView = user.isAdmin() ? 
	                new ModelAndView("admin-dashboard") :
	                new ModelAndView("user-dashboard");
	            modelAndView.addObject("message", "Welcome " + (user.isAdmin() ? "Admin" : "") + ", " + user.getName() + "!");
	        } else {
	            // User was not found even though they are authenticated
	            modelAndView = new ModelAndView("login");
	            modelAndView.addObject("message", "User not found in the database.");
	        }
	    } else {
	        modelAndView = new ModelAndView("login");
